...
namespace WeChatCRM.Common.Utils
{
    ...
    public sealed class NetHelper
    {
        ...
        public static string HttpRequest(string url, string data, string method, 
            int timeout, Encoding encoding, string contentType)
        {
        ...
        public static string HttpUploadFile(string url, string fileKey, Dictionary<string, object> nvc, List<FileUploadInfo> fileList)
        {
            ...
            foreach (var fileUpload in fileList)
            {
                //分隔符
                postStream.Write(boundarybytes, 0, boundarybytes.Length);
                try
                {
                    //读取文件流
                    Stream fileStream = fileUpload.Stream;// new FileStream(file, FileMode.Open, FileAccess.Read);
                    //文件头部
                    string fileName = Path.GetFileName(fileUpload.FileName);
                    string header = string.Format(FileHeaderTemplate, fileKey, fileName, fileUpload.ContentType);
    ...
    public class FileUploadInfo
    {
        ...
        public string ContentType
        {
            get
            {
                return "image/" + Path.GetExtension(this.FileName);
            }
        }
