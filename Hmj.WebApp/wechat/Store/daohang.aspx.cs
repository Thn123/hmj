...
function createMap(){
        ...
        {
            List<MDSearch> list = mss.GetCity(DropDownList1.SelectedValue, int.Parse(DropDownList4.SelectedValue));
     ...
     ";
       ...
       <p><span><a href='http://api.map.baidu.com/place/search?query={4}&location={0},{1}&radius=1000&region={5}&output=html&src=yourCompanyName|yourAppName'>查看地图</a></span><strong style='font-size:12px;'>{6}</strong></p>
        ...
        {
            List<MDSearch> list = mss.GetCity(DropDownList3.SelectedValue, int.Parse(DropDownList4.SelectedValue));
            DataTable dt = ListToDataTable(list);
            DropDownList1.Items.Clear();
